...
//!   once per session, so [`PortalHotkeys::register`] closes the old session
//!   and opens a fresh one every time.
...
/// Our one shortcut's portal id. It is persisted by the portal against this
/// string, so changing it orphans every existing user's binding — treat it as
/// a wire format, not a label.
...
const SHORTCUT_DESCRIPTION: &str = "Capture a screenshot";
...
/// How long [`PortalHotkeys::register`] waits for the bind result before
...
enum Cmd {
    /// Bind `trigger`, replacing whatever session is live. The outcome goes
    /// back over `reply` — `Ok` once the portal has accepted the request.
    Bind {
        trigger: String,
...
impl PortalHotkeys {
    ...
    /// Ask the compositor to bind `hk`, replacing any previous request.
    ///
    /// `Ok` means the portal accepted the request — *not* that the key is
    /// live, which only the compositor can decide. See the module docs.
    pub fn register(&mut self, hk: Hotkey) -> Result<()> {
        let trigger = portal_trigger(&hk).ok_or_else(|| Error::Hotkey(format!(
        ...
        self.cmds
            .send(Cmd::Bind { trigger, reply: reply_tx })
            .map_err(|_| Error::Hotkey("the portal thread stopped".into()))?;

        match reply_rx.recv_timeout(BIND_TIMEOUT) {
            Ok(Ok(()))   => Ok(()),
            Ok(Err(msg)) => Err(Error::Hotkey(msg)),
    ...
    /// Take every press seen since the last call, collapsing a burst into one.
    /// Matches `HotkeyManager::drain_pressed`'s contract so the winit loop
        ...
        while let Ok(cmd) = cmds.recv() {
            match cmd {
                Cmd::Bind { trigger, reply } => {
                    ...
                    let request = NewShortcut::new(SHORTCUT_ID, SHORTCUT_DESCRIPTION)
                        .preferred_trigger(trigger.as_str());

                    let outcome = match shortcuts.bind_shortcuts(&fresh, &[request], None).await {
...
/// Forwards `Activated` signals for our shortcut id into `presses`.
///
...
fn listen_loop(presses: Sender<()>) {
    pollster::block_on(async move {
        ...
        while let Some(event) = stream.next().await {
            ...
            if event.shortcut_id() == SHORTCUT_ID && presses.send(()).is_err() {
                return;
            }
        }
