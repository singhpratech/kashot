...
impl Overlay {
    ...
    pub fn new_for_region_record(
        ...
        let mut me = Self::build(loop_target, screenshot, settings, false, None, None)?;
        me.record_mode = true;
        Ok(me)
    }
    ...
    fn handle_left_press(&mut self) -> OverlayOutcome {
        ...
        if self.state == State::Selected {
            if let Some(sel) = self.selection {
                ...
                // Action panel.
                let ap_origin = action_panel_origin(win_w, win_h, sel, self.action_buttons());
    ...
    fn compute_hover_tip(&self) -> Option<(&'static str, i32, i32)> {
        ...
        // Action panel.
        let ap = action_panel_origin(win_w, win_h, sel, self.action_buttons());
    ...
    /// the session is the rectangle itself. Window pixels — the caller clamps
    /// them to the desktop and rounds them for the encoder.
    fn commit_as_record(&mut self) -> OverlayOutcome {
        if self.state != State::Selected { return OverlayOutcome::Continue; }
        match self.selection {
            Some(rect) => OverlayOutcome::RecordRegion(rect),
            None       => OverlayOutcome::Continue,
        }
    }
    ...
    fn redraw(&mut self) {
        ...
        if matches!(self.state, State::Selected | State::Drawing | State::Resizing | State::TextInput) {
            if let Some(sel) = self.selection {
                ...
                draw_action_panel(&mut buf, win_w, win_h, sel, actions);
                ...
                if matches!(self.state, State::Selected) {
                    if let Some(start) = self.panel_pulse_started {
                        ...
                        if elapsed < 3.0 {
                            draw_action_panel_pulse(&mut buf, win_w, win_h, sel, elapsed, actions);
                        } else {
...
fn draw_action_panel_pulse(
    ...
    let origin = action_panel_origin(win_w, win_h, sel, buttons);
    let (pw, ph) = action_panel_dims(buttons);
...
fn draw_idle_hint(buf: &mut [u32], win_w: usize, win_h: usize, hint: &str) {
    const SCALE:  i32 = 2;
    const PAD_X:  i32 = 12;
    const PAD_Y:  i32 = 8;
