...
| R10 | Bundled Wayland support | ✅ **shipped** — session type is detected at launch (`kashot_platform::session`) and each X11-only mechanism has a portal counterpart via `ashpd`: global hotkey through GlobalShortcuts (`hotkey_portal.rs`), screenshot through `xcap`'s portal backends with a Screenshot-portal fallback (`capture_portal.rs`), recording through ScreenCast → PipeWire → ffmpeg stdin (`recorder_wayland.rs`). The X11 key grab is never used as a Wayland fallback — XWayland answers it and the key then silently never fires. | Hardware-verify on GNOME / KDE / wlroots Wayland; persist the ScreenCast restore token across runs so the share prompt appears once ever rather than once per launch |
