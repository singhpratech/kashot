...
| R3 | Screen recording | ✅ **shipped** in Rust (v0.3.0) on Linux X11 (`ffmpeg -f x11grab` + PulseAudio mic + monitor source), Windows (`ffmpeg -f gdigrab` + DirectShow mic — system-audio loopback queued), and macOS (built-in `screencapture -v` for video; `ffmpeg -f avfoundation` for mic since v0.3.10, system audio via a loopback/Aggregate device). Floating REC indicator with STOP. | Wayland capture via `xdg-desktop-portal` (ashpd); WASAPI loopback on Windows; native AVFoundation system-audio tap on macOS. |
