...
*Last updated: 2026-05-25 (v0.4.2 — one-click **Capture Full Screen** tray item that grabs the whole desktop and auto-saves it (watermark applied, no region selector or editor), plus a recording robustness fix: the encoder is now polled after spawn so a screen/audio recorder that exits immediately (missing pulse demuxer, unopenable device, Wayland display) surfaces an actionable error instead of a silent "started" with no file, and Recorder::stop() verifies the output file exists. v0.4.1 — Windows recording polish: ffmpeg/PowerShell helper consoles are now hidden (CREATE_NO_WINDOW), a missing/denied microphone gives an actionable message instead of a raw HRESULT, and the recording-failure dialog no longer blocks screen capture. v0.4.0 had landed native system-audio capture on Windows (WASAPI loopback) and macOS (ScreenCaptureKit), each streamed into ffmpeg over a loopback socket, so recording system sound no longer needs a Stereo Mix / BlackHole driver; Linux already captured system audio via the PulseAudio monitor source. Earlier in v0.3.12: the bundled Linux ffmpeg switched to a BtbN static GPL build with PulseAudio support so Linux audio capture works). Maintained by [@singhpratech](https://github.com/singhpratech).*
