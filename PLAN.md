...
*Last updated: 2026-05-25 (v0.4.0 release — native system-audio capture on Windows (WASAPI loopback) and macOS (ScreenCaptureKit), each streamed into ffmpeg over a loopback socket, so recording system sound no longer needs a Stereo Mix / BlackHole driver; Linux already captured system audio via the PulseAudio monitor source. Earlier in v0.3.12: the bundled Linux ffmpeg switched to a BtbN static GPL build with PulseAudio support so Linux audio capture works). Maintained by [@singhpratech](https://github.com/singhpratech).*
